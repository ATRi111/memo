...
$$
\textbf{零化多项式}:若P(\mathbf A) \equiv 0 \quad 特征多项式(\lambda替换成\mathbf A)一定是零化多项式 \hfill \\
...
\\
单步\mathrm{Euler}法:y_{n+1}=y_n+hg(x_n,y_n,y_{n+1}) \quad g:增量函数\hfill \\
\textbf{局部截断误差}:T_{n+1}=y(x_{n+1})-y(x_n) \hfill \\
若某种算法的任意一步误差均满足\mathrm O(h^p),称其具有p-1阶精度 \hfill \\
\\
假设初始条件中存在误差E_0=y_0-\overline{y_0},后续误差E_n=y_n-\overline{y_n} \hfill \\
取y'=\lambda y,若\exists h,使E_{n+1} < E_n,则算法绝对稳定;使算法稳定的所有h构成绝对稳定区域 \hfill \\
\\
