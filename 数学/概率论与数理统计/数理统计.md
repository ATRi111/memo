...
2.构造统计量Y,并确定一个\textbf{显著性水平}\alpha \hfill \\ 
3.根据Y确定一个接受H_0的条件\hfill \\
\\
...
| $H_0$             | 接受$H_0$的条件                         |
| ----------------- | --------------------------------------- |
| $\theta=\theta_0$ | $Y_{1-\alpha/2} \le Y \le Y_{\alpha/2}$ |
| $\theta>\theta_0$ | $Y > Y_{1-\alpha}$                      |
| $\theta<\theta_0$ | $Y < Y_{\alpha}$                        |

**$Y$的选取参考区间估计，将区间估计的$Y$中的待估参数替换成$H_0$中规定的值**

- **试图证明某个结论时，应将该结论的反面定为假设，并确定一个充分小的显著性水平，然后拒绝假设让结论成立，这样才有说服力**

## 题型：假设检验

$$
X \sim N(\mu,0.01),对X抽样,得到10个样本观测值,\overline{x}=10.05,能否认为\mu=10(\alpha=0.05)? \hfill \\
则H_0的置信度为0.95的置信区间为\left(\overline{X}-\frac{0.1}{\sqrt{10}}u_{0.025},\overline{X}+\frac{0.1}{\sqrt{10}}u_{0.025}\right) =(9.988,10.112) \hfill \\
故能认为\mu=10 \hfill \\
$$
