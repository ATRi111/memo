...
- 在三维图形中，一个顶点被多个面包含，因此可能“同时拥有”多个纹理坐标
  - 有纹理的长方体需要14个顶点来表示（参考展开图）
...
- Draw Call：CPU向GPU发出的一次绘制指令，如`glDrawElements`
- 减少Draw Call能有效提高渲染效率，主要方法是Batch（批量渲染）
  - 将多次渲染使用的多个VAO/VBO/IBO合并为一个VAO/VBO/IBO
  - 要将不同纹理的Draw Call合批，则需要在VBO中储存纹理插槽，并在着色器中对各个插槽的纹理采样
