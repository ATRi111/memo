...
D(\mathbf n,\mathbf l) = \frac{\alpha(\mathbf p)^2}{\pi((\mathbf n \cdot \mathbf h)^2(\alpha^2-1)+1)^2} \quad 其中\mathbf h =\frac{ \mathbf l + \mathbf v}{|\mathbf l+\mathbf v|} \hfill \\
F(\mathbf v,\mathbf l) = \mathrm{Lerp}\{F_0(\mathbf p),1,(1 - \mathbf v \cdot \mathbf h)^5\}=F_0(\mathbf p) + (1-F_0(\mathbf p))(1 - \mathbf v \cdot \mathbf h)^5 \hfill \\
G(\mathbf v,\mathbf n,\mathbf l) = g(\mathbf l)g(\mathbf v) \quad 其中 g(\mathbf x) = \frac{\mathbf n \cdot \mathbf x}{(1-k)\mathbf n \cdot \mathbf x + k} \quad k = \frac{(\alpha(\mathbf p) + 1)^2}{8} \hfill \\
\\
D:描述法线的分散程度 \quad F:描述菲涅尔现象的强度 \quad G:描述表面凹凸带来的吸光能力(值越大,吸光能力越弱) \quad \hfill \\
\alpha(\mathbf p):\mathbf p处的粗糙度,[0,1] \quad F_0(\mathbf p):\mathbf p处的菲涅尔反射率 \hfill \\
$$
...
- Trowbridge-Reitz(GGX)模型中的菲涅尔反射率较难理解，不利于创作，Metallic PBR材质对其进行了简化和包装：
