...
\\
对于只考虑漫反射的情况,L_r(\mathbf p,\mathbf v)= \int_{\Omega^{+}} L(\xi,-\mathbf l) (\mathbf l \cdot \mathbf n) k_d \mathrm d\omega=k_dE(\mathbf p) \hfill \\
E(\mathbf p):所有光源,反射源在\mathbf p产生的光照度(已考虑照射角) \hfill \\
此情况下k_d=\frac{\rho}{\pi} \quad \rho:\mathbf p点处的\textbf{兰伯特反射率} \hfill \\
恒有0 \le \rho \le 1 \quad \rho = 1时,入射光被完全反射,总能量不变 \hfill \\ 
...
\underset{direct}{L_{r}(\mathbf p,\mathbf v)}= E(\mathbf p) [k_d(\mathbf l \cdot \mathbf n) + k_s(\mathbf n \cdot \mathbf h)^g] \quad 其中\mathbf h =\frac{ \mathbf l + \mathbf v}{|\mathbf l+\mathbf v|} \hfill \\
\underset{direct}{L_{r}(\mathbf p,\mathbf v)}:直接光照产生的反射光亮度 \quad E(\mathbf p):光源\textbf{直射} \mathbf p处产生的光照度\hfill \\
...
$$
规定f(\mathbf l,\mathbf v,\mathbf n) = \frac{\rho}{\pi} + \frac{D(\mathbf n,\mathbf l)F(\mathbf n,\mathbf l)G(\mathbf p,\mathbf v,\mathbf l)}{4 \ (\mathbf l \cdot \mathbf n)(\mathbf v \cdot \mathbf n)} \hfill \\
给定物体表面一点\mathbf p,考虑单一光源:\underset{direct}{L_{r}(\mathbf p,\mathbf v)}= E(\mathbf p) (\mathbf l \cdot \mathbf n) \left[\frac{\rho}{\pi} + \frac{D(\mathbf p,\mathbf v,\mathbf l)F(\mathbf n,\mathbf l)G(\mathbf p,\mathbf v,\mathbf l)}{4 \ (\mathbf l \cdot \mathbf n)(\mathbf v \cdot \mathbf n)} \right] \hfill \\
D(\mathbf p,\mathbf v,\mathbf l):法线的分散情况(法线方向越集中,且越集中在半程向量方向时,向观察方向反射得越多) \hfill \\
F(\mathbf p,\mathbf v,\mathbf l):不考虑微表面几何,射入的光线被反射的比例(大体上,观察方向与入射方向夹角越大,反射越多) \hfill \\
G(\mathbf p,\mathbf v,\mathbf l):入射和反射光线不被微表面结构遮挡的比例(入射方向/观察方向与法线夹角越小,未被遮挡越多) \hfill \\
\alpha:\mathbf p处的粗糙度,[0,1] \quad \rho:\mathbf p处的\textbf{兰伯特反射率} \hfill \\
\\
F(\mathbf p,\mathbf v,\mathbf l) \approx \mathrm{Lerp}\{F_0,1,(1 - \mathbf v \cdot \mathbf h)^5\}=F_0 + (1-F_0)(1 - \mathbf v \cdot \mathbf h)^5 \hfill \\
F_0:\mathbf p处的\textbf{菲涅尔反射率}(入射方向,观察方向,法线方向共线时的F项) \hfill \\
F_0=\frac{(n_1-n_2)^2}{(n_1+n_2)^2} \quad n_1,n_2:反射面两侧两种物质的折射率 \hfill \\
$$

- **Microfacet模型**定义了BRDF的一套标准，符合这套标准的BRDF即称为Microfacet BRDF
  - **F项有通用的近似方式，D项,G项则有多种近似**

$$
\mathrm{Beckmann \ NDF}:D(\mathbf p,\mathbf v,\mathbf l)=\frac{e^{-\tan^2{\theta_h}/\alpha^2}}{\pi \alpha^2\cos^4\theta_h} \quad \cos \theta_h=\mathbf n \cdot \mathbf h \hfill \\
\mathrm{Trowbridge-Reitz(GGX) \ NDF}:D(\mathbf p,\mathbf v,\mathbf l) = \frac{\alpha^2}{\pi((\mathbf n \cdot \mathbf h)^2(\alpha^2-1)+1)^2} \hfill \\
$$

- D项即Normal Distribution Function(法线分布函数)

$$
\mathrm{Smith \ SMT}:G(\mathbf p,\mathbf v,\mathbf l) = g(\mathbf l)g(\mathbf v) \quad 其中 g(\mathbf x) = \frac{\mathbf n \cdot \mathbf x}{(1-k)\mathbf n \cdot \mathbf x + k} \quad k = \frac{(\alpha + 1)^2}{8} \hfill \\
$$
...
<img src="image-20250901122653249.png" alt="image-20250901122653249" style="zoom:50%;" />

*上图为同一种物质，粗糙度不同，粗糙度高时总体过于暗*

- Microfacet模型中，认为光线被微表面结构遮挡后就消失了，这会导致能量减少（主要由G项引起）

$$
L_r(\mathbf p,\mathbf v)=\int_{\Omega^{+}} L(\xi,-\mathbf l) (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\omega \hfill \\
假设各方向光线均匀射入,则L_r(\mathbf p,\mathbf v)=L\int_{\Omega^{+}} (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\omega
=\int_0^{2\pi}\mathrm d \theta \int_0^{\pi/2} (\mathbf l \cdot \mathbf n) f(\mathbf p,\theta,\phi,\mathbf v) \sin \phi \mathrm d\phi \hfill \\ 
注意到\mathbf l \cdot \mathbf n=\cos (\frac{\pi}{2}-\phi)=\cos \phi,则L_r(\mathbf p,\mathbf v)=L\int_0^{2\pi}\mathrm d \theta \int_0^1 (\mathbf l \cdot \mathbf n) f(\mathbf p,\theta,\phi,\mathbf v) \sin \phi \mathrm d \sin\phi \hfill \\
在一个方向上损失的亮度=L-L_r(\mathbf p,\mathbf v) \hfill \\
\\
假设
$$
...
- Micro-facet模型中的菲涅尔反射率较难理解，不利于创作，Metallic PBR材质对其进行了简化和包装：
  ...
  - 材质中的各个贴图规定的是特定分辨率下的法线方向和粗糙度，分辨率降低后，(单一像素内)法线的分散程度可能会提高
...
$$
\underset{env\_specular}{L_r(\mathbf p,\mathbf v)}= \int_{\Omega^{+}} L_r(\mathbf o_e,-\mathbf l)V(\mathbf p,\mathbf l) \frac{D(\mathbf n,\mathbf l)F(\mathbf n,\mathbf l)G(\mathbf p,\mathbf v,\mathbf l)}{4 \ (\mathbf v \cdot \mathbf n)} \mathrm d\omega \hfill \\
\approx L_r(\mathbf o_e,-\mathbf r) \int_{\Omega^{+}} \frac{D(\mathbf n,\mathbf l)F(\mathbf n,\mathbf l)G(\mathbf p,\mathbf v,\mathbf l)}{4 \ (\mathbf v \cdot \mathbf n)} \mathrm d\omega  \hfill \\
...
D(\mathbf p,\mathbf v,\mathbf l) = \frac{\alpha^2}{\pi((\mathbf n \cdot \mathbf h)^2(\alpha^2-1)+1)^2} \quad 其中\mathbf h =\frac{ \mathbf l + \mathbf v}{|\mathbf l+\mathbf v|} \hfill \\
F(\mathbf p,\mathbf v,\mathbf l) = \mathrm{Lerp}\{F_0,1,(1 - \mathbf v \cdot \mathbf h)^5\}=F_0 + (1-F_0)(1 - \mathbf v \cdot \mathbf h)^5 \hfill \\
G(\mathbf p,\mathbf v,\mathbf l) = g(\mathbf l)g(\mathbf v) \quad 其中 g(\mathbf x) = \frac{\mathbf n \cdot \mathbf x}{(1-k)\mathbf n \cdot \mathbf x + k} \quad k = \frac{(\alpha + 1)^2}{8} \hfill \\
\\
...
\underset{env\_specular}{L_r(\mathbf p,\mathbf v)}=L_r(\mathbf o_e,-\mathbf r) \int_{\Omega^{+}} \frac{D(\cos \theta,\alpha) [F_0 + (1-F_0)(1 - \cos \theta)^5]G(\cos \theta,\alpha)}{4 \cos \theta}\mathrm d\omega \hfill \\
=L_r(\mathbf o_e,-\mathbf r)\left[ F_0\int_{\Omega^{+}} \frac{D(\cos \theta,\alpha) [1-(1 - \cos \theta)^5]G(\cos \theta,\alpha)}{4 \cos \theta}\mathrm d\omega+\int_{\Omega^{+}} \frac{D(\cos \theta,\alpha) (1 - \cos \theta)^5G(\cos \theta,\alpha)}{4 \cos \theta}\mathrm d\omega \right] \hfill \\
=L_r(\mathbf o_e,-\mathbf r)\left[F_0\int_{\Omega^{+}} g(\alpha,\cos \theta) \mathrm d\omega+\int_{\Omega^{+}} h(\alpha,\cos \theta) \mathrm d\omega  \right]\hfill \\
...
\underset{indirect}{L_r(\mathbf p,\mathbf v)}=k_dL \int_{\Omega^{+}} V(\mathbf p,\mathbf l) (\mathbf l \cdot \mathbf n) \mathrm d\omega \approx k_d\int_{\Omega^{+}} V(\mathbf p,\mathbf l) \mathrm d\omega \hfill \\
$$
