...
\\
F_0=\frac{(n_1-n_2)^2}{(n_1+n_2)^2} \hfill \\
F_0:\mathbf p处的\textbf{菲涅尔反射率}  \quad n_1,n_2:反射面两侧两种物质的\textbf{折射率(IoR)} \hfill \\
$$
...
- **Cook-Torrance模型**将镜面反射分为D,F,G三项，**没有考虑多次反射**，认为光线一旦被遮挡就消失
  ...
  - D项即**法线分布函数(半程向量和前面的微表面法线就是同一个量，因为只有这个方向的微表面能实现特定光路的镜面反射)**
...
$$
  ...
  - 对Microfacet模型的封装，同样符合Disney Principled BRDF，但参数数量少很多
  ...
  - Roughness图规定的是特定分辨率下粗糙度，分辨率降低后，从Microfacet模型来考虑，实际粗糙度应当提高
