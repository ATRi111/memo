...
L_r(\mathbf p,\mathbf v)= \int_{\Omega^{+}} L(-\mathbf l,\mathbf p) (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\
\\
...
=\int_{\Omega^{+}} L(-\mathbf l,\mathbf p) (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\
...
\\
如果物体没有镜面反射: \hfill \\
L_r(\mathbf p,\mathbf v)= \int_{\Omega^{+}} L(-\mathbf l,\mathbf p) (\mathbf l \cdot \mathbf n) \frac{\rho}{\pi} \mathrm d\mathbf l=\frac{\rho}{\pi}E(\rightarrow\mathbf p) \hfill \\
...
E(\mathbf p\rightarrow)=\int_{\Omega^{+}} \frac{\rho}{\pi}E(\rightarrow\mathbf p) \cos\phi \mathrm d\mathbf l_o=E(\rightarrow\mathbf p) \rho \hfill \\
\rho = 1时,E(\mathbf p\rightarrow)=E(\rightarrow\mathbf p),总能量不变 \hfill \\ 
注意:\int_{\Omega^{+}} \mathrm d\mathbf l=2\pi,\int_{\Omega^{+}} \cos\phi \mathrm d\mathbf l=\pi \quad \cos\phi \mathrm d\mathbf l表示立体角投影,其积分即表示半球面投影而得的圆的面积\hfill \\
$$
...
$$
\int_a^b f(x)g(x) \mathrm dx \approx \frac{\int_a^b f(x) \mathrm dx}{\int_a^b\mathrm dx}\int_a^b g(x) \mathrm dx \hfill \\
f(x)或g(x)任意一个在[a,b]内的取值变化较小时,此近似较为准确 \hfill \\
\\
\int_a^b f(x) \mathrm d x \int_c^d g(y) \mathrm dy \approx \int_a^b f(x) \mathrm d x \cdot \int_c^d g(y) \mathrm dy \hfill \\
f(x)或g(y)任意一个在定义域内的取值变化较小时,此近似较为准确  \hfill \\
$$
...
$$
L_r(\mathbf p_o,\mathbf v)=\int_A \mathrm d\mathbf p_i \int_{\Omega^{+}} L(-\mathbf l,\mathbf p_i) (\mathbf l \cdot \mathbf n) f(\mathbf p_i,\mathbf p_o,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\
A:物体表面 \quad \mathbf p_i:光线入射点 \quad \mathbf p_o:光线出射点 \hfill \\
$$
...
$$
定义D(\mathbf p,\mathbf h)=\frac{\mathrm dA_h}{A\mathrm d\mathbf h}=\frac{(\mathbf n \cdot \mathbf h) \mathrm dA}{A\mathrm d\mathbf h} \hfill \\
\Rightarrow \int_{\Omega^+} (\mathbf n \cdot \mathbf h) D(\mathbf h) \mathrm d \mathbf h = \int_A \frac{1}{A} \mathrm dA = 1 \hfill \\
A:\mathbf p附近一块宏观上平坦的表面(的面积) \quad \mathbf n:宏观法线 \quad \mathbf h:某处微表面的法线 \hfill \\
 \mathrm d\mathbf h:\mathbf h \textbf{附近}的微小立体角 \quad \mathrm d A_h:法线方向落在\mathbf h\textbf{附近}的所有微表面的总面积 \hfill \\
$$
...
\end{bmatrix} \hfill \\
任取一组三维正交基\omega_1,\omega_2,\omega_3,记\mathbf P = \begin{bmatrix}\mathbf \omega_1 & \mathbf \omega_2 & \omega_3 \end{bmatrix},则\mathbf P^{-1}=\mathbf P^T \hfill \\ 
...
\end{bmatrix} \hfill \\
\\
如果\mathbf S能用于表示法线分布: \hfill \\
D(\mathbf S,\mathbf h)=\frac{1}{\pi \sqrt{|\mathbf S|}(\mathbf h^T\mathbf S^{-1}\mathbf h)^2} \hfill \\
\sigma(\mathbf S,\mathbf x)=\int_\Omega \langle \mathbf x,\mathbf h \rangle D(\mathbf h) \mathrm d \mathbf h = \sqrt{\mathbf x^T\mathbf S\mathbf x} \hfill \\
\sigma(\mathbf S,\mathbf x):\mathbf p附近所有微表面在垂直于\mathbf x的平面内的投影面积 \hfill \\
$$
...
$$
上图所示情况下,\sigma(\omega_1)=\sigma(\omega_2)=\sigma,\sigma(\omega_3)=1,记\omega_3=(x,y,z), \hfill \\
...
\end{bmatrix} \hfill \\
\\
例如,\mathrm{GGX}模型认为D(\mathbf p,\mathbf h) \approx \frac{\alpha^2}{\pi((\mathbf n \cdot \mathbf h)^2(\alpha^2-1)+1)^2} \hfill \\
$$
...
$$
上图所示情况下,\sigma(\omega_1)=\sigma(\omega_2)=1,\sigma(\omega_3)=\sigma,记\omega_3=(x,y,z), \hfill \\
...
$$
D(\mathbf p,\mathbf h)=\sum_i w_i D_i(\mathbf S_i,\mathbf h) \approx D_i(\sum_i w_i\mathbf S_i,\mathbf h) \quad 其中\sum_i w_i = 1\hfill \\
$$
...
$$
给定物体表面一点\mathbf p,考虑单一光源,规定f(\mathbf p,\mathbf l,\mathbf v) = k_d + k_s\frac{(\mathbf n \cdot \mathbf h)^g}{(\mathbf l \cdot \mathbf n)}: \hfill \\
...
$$
\mathrm{Cook-Torrance}模型:f(\mathbf p,\mathbf l,\mathbf v) = f_d(\mathbf p,\mathbf l,\mathbf v) + \frac{D(\mathbf h;\mathbf n,\alpha)F(\mathbf v,\mathbf h;F_0)G(\mathbf l,\mathbf v;\mathbf n,\alpha)}{4 \ (\mathbf l \cdot \mathbf n)(\mathbf v \cdot \mathbf n)} \hfill \\
给定物体表面一点\mathbf p,考虑单一光源:\underset{direct}{L_{r}(\mathbf p,\mathbf v)}= E(-\mathbf l,\mathbf p) (\mathbf l \cdot \mathbf n) \left[f_d(\mathbf p,\mathbf l,\mathbf v) + \frac{D(\mathbf h;\mathbf n,\alpha)F(\mathbf v,\mathbf h;F_0)G(\mathbf l,\mathbf v;\mathbf n,\alpha)}{4 \ (\mathbf l \cdot \mathbf n)(\mathbf v \cdot \mathbf n)} \right] \hfill \\
D(\mathbf h;\mathbf n,\alpha):法线的分散情况(法线方向越集中,且越集中在半程向量方向时,向观察方向反射得越多) \hfill \\
F(\mathbf v,\mathbf h;F_0):不考虑微表面几何,射入的光线被反射的比例(大体上,观察方向与入射方向夹角越大,反射越多) \hfill \\
G(\mathbf l,\mathbf v;\mathbf n,\alpha):入射和反射光线不被微表面结构遮挡的比例(入射方向/观察方向与法线夹角越小,未被遮挡越多) \hfill \\
r:\mathbf p处的粗糙度,[0,1],通常取\alpha = r^2 \quad \rho:\mathbf p处的\textbf{兰伯特反射率},[0,1] \hfill \\
\\
对于\textbf{特定波长}的光:F_0=\frac{(n_1-n_2)^2}{(n_1+n_2)^2} \hfill \\
F_{\theta}:\mathbf p处,\mathbf l,\mathbf v与\mathbf h的夹角为\theta时的\textbf{菲涅尔反射率}  \quad n_1,n_2:反射面两侧两种物质对\textbf{特定波长}的光的\textbf{折射率(IoR)} \hfill \\
$$
...
$$
\mathrm{Lambert \ Diffuse}:f_d(\mathbf p,\mathbf l,\mathbf v)= (1-F(\mathbf v,\mathbf h;F_0))\frac{\rho}{\pi} \hfill \\
\mathrm{Renormalized \ Disney \ Diffuse}:f_d(\mathbf p,\mathbf l,\mathbf v)= s(\mathbf l;\mathbf n,r)s(\mathbf v;\mathbf n,r)\eta (r)\frac{\rho}{\pi} \hfill \\
其中s(\mathbf x;\mathbf n,r)=\mathrm{Lerp}\{1,\frac{r}{2}+2(\mathbf x \cdot \mathbf h)^2r,(1 - \mathbf x \cdot \mathbf n)^5\} \quad \eta(r) = \mathrm{Lerp}\{1,\frac{1}{1.51},r\}  \hfill \\
\\
\mathrm{Schlick \ Fresnel}:F(\mathbf v,\mathbf h;F_0) \approx \mathrm{Lerp}\{F_0,F_{90},(1 - \mathbf v \cdot \mathbf h)^5\}=F_0 + (1-F_0)(1 - \mathbf v \cdot \mathbf h)^5 \hfill \\
\\
\mathrm{Beckmann \ NDF}:D(\mathbf h;\mathbf n,\alpha)=\frac{e^{-\tan^2{\theta_h}/\alpha^2}}{\pi \alpha^2\cos^4\theta_h} \quad \cos \theta_h=\mathbf n \cdot \mathbf h \hfill \\
\mathrm{Trowbridge-Reitz(GGX) \ NDF}:D(\mathbf h;\mathbf n,\alpha) = \frac{\alpha^2}{\pi((\mathbf n \cdot \mathbf h)^2(\alpha^2-1)+1)^2} \hfill \\
\\
\mathrm{Smith \ SMT}:G(\mathbf l,\mathbf v;\mathbf n,\alpha) = \frac{1}{\,1 + g(\mathbf l) + g(\mathbf v)} \quad 
 g(\mathbf x)= \frac{1}{2}\left(\frac{\sqrt{\alpha^2 + (1-\alpha^2)(\mathbf n \cdot \mathbf x)^2}}{\mathbf n \cdot \mathbf x}- 1\right) \hfill \\
\mathrm{Schlick-Smith \ SMT}:G(\mathbf l,\mathbf v;\alpha,\mathbf n) = \Lambda(\mathbf l)\Lambda(\mathbf v) \quad 其中 \Lambda(\mathbf x) = \frac{\mathbf n \cdot \mathbf x}{(1-k)\mathbf n \cdot \mathbf x + k} \quad k = \frac{(\alpha + 1)^2}{8} \hfill \\
$$
...
记\eta(\mathbf p,\mathbf l)=\frac{E(\mathbf p\rightarrow)}{E(-\mathbf l,\mathbf p)}=\frac{1}{E(-\mathbf l,\mathbf p)}\int_{\Omega^+} L_r(\mathbf p,\mathbf v) \cos \phi \mathrm d \mathbf v = \int_{\Omega^+} f(\mathbf p,\mathbf l,\mathbf v) \cos\theta_o \mathrm d \mathbf v \hfill \\
...
\\
假设F(\mathbf v,\mathbf h;F_0) \equiv 1(无漫反射,无吸收): \hfill \\
...
$$
F(\mathbf v,\mathbf h;F_0) < 1(有漫反射)时: \hfill \\
...
$$
对于有体积均匀光源:L_r(\mathbf p,\mathbf v)= L\int_{\Omega} (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\omega \hfill \\
认为f(\mathbf p,\mathbf l,\mathbf v)和\mathbf v给定时,\forall \Omega,\exists \mathbf M_{3 \times 3}(线性变换,不需要齐次矩阵), \hfill \\
使得L_r(\mathbf p,\mathbf v)=
L\int_{\mathbf M\Omega} \cos\phi  \mathrm d  \frac{\mathbf M \mathbf l}{|\mathbf M\mathbf l|}=L\int_{\mathbf M\Omega} \cos\phi |\mathbf J| \mathrm d \mathbf l \quad \mathbf J=\frac{1}{|\mathbf M\mathbf l|^3}\mathbf M \hfill \\
\Omega:光源覆盖的球面区域 \quad \phi:\mathbf M\mathbf l与天顶(\mathbf n)的夹角 \quad \mathbf J:\mathrm{Jacobian}矩阵 \hfill \\
$$
...
$$
F_0 = \frac{(n_1-n_2)^2}{(n_1+n_2)^2} \quad 默认情况下,取n_1 = 1,n_2 = 1.5 \hfill \\
\mathbf F_0=\mathrm{Lerp}\{F_0 \cdot \mathbf 1 ,\mathbf A,m\} \hfill \\
\mathbf F_0:\mathrm{RGB}模型下的菲涅尔反射率 \quad \mathbf A:\mathrm{Albedo} \quad m:金属度 \hfill \\
$$
...
\approx \overline{V_i}\sum_i E(\mathbf o_i\rightarrow\mathbf p)(\mathbf l_i \cdot \mathbf n) f(\mathbf p,\mathbf l_i,\mathbf v) \hfill \\
$$
...
$$
环境映射:\underset{env}{L_r(\mathbf p,\mathbf v)}= \int_{\Omega^{+}} L_r(\mathbf o_e,-\mathbf l)V(\mathbf p,\mathbf l) (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\
...
$$
\underset{env\_specular}{L_r(\mathbf p,\mathbf v)}= \int_{\Omega^{+}} L_r(-\mathbf l,\mathbf p)V(\mathbf p,\mathbf l) \frac{D(\mathbf n,\mathbf l)F(\mathbf v,\mathbf h;F_0)G(\mathbf l,\mathbf v;\mathbf n,\alpha)}{4 \ (\mathbf v \cdot \mathbf n)} \mathrm d\mathbf l \hfill \\
\approx L_r(-\mathbf r,\mathbf p) \int_{\Omega^{+}} \frac{D(\mathbf n,\mathbf l)F(\mathbf v,\mathbf h;F_0)G(\mathbf l,\mathbf v;\mathbf n,\alpha)}{4 \ (\mathbf v \cdot \mathbf n)} \mathrm d\mathbf l  \hfill \\
\mathbf r:\mathbf v关于\mathbf n的镜面对称方向 \hfill \\
\\
以\mathrm{Trowbridge-Reitz(GGX) \ NDF}为例: \hfill \\
D(\mathbf h;\mathbf n,\alpha) = \frac{\alpha^2}{\pi((\mathbf n \cdot \mathbf h)^2(\alpha^2-1)+1)^2} \quad 其中\mathbf h =\frac{ \mathbf l + \mathbf v}{|\mathbf l+\mathbf v|} \hfill \\
F(\mathbf v,\mathbf h;F_0) = \mathrm{Lerp}\{F_0,1,(1 - \mathbf v \cdot \mathbf h)^5\}=F_0 + (1-F_0)(1 - \mathbf v \cdot \mathbf h)^5 \hfill \\
G(\mathbf l,\mathbf v;\mathbf n,\alpha) = g(\mathbf l)g(\mathbf v) \quad 其中 g(\mathbf x) = \frac{\mathbf n \cdot \mathbf x}{(1-k)\mathbf n \cdot \mathbf x + k} \quad k = \frac{(\alpha + 1)^2}{8} \hfill \\
\\
...
$$
只考虑自遮挡和自反射:L_r(\mathbf p,\mathbf v)= \int_{\Omega^{+}} L(-\mathbf l,\mathbf p) (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\
...
\\
只考虑环境光直射时:L_r^0(\mathbf p,\mathbf v)=\int_{\Omega^{+}} L_e(-\mathbf l,\mathbf p)V(\mathbf p,-\mathbf l) (\mathbf l \cdot \mathbf n)f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\
考虑直射及前i次反射时:L_r^i(\mathbf p,\mathbf v)
=L_r^0(\mathbf p,\mathbf v)+\int_{\Omega^{+}} L_r^{i-1}(-\mathbf l,\mathbf p)[1-V(\mathbf p,-\mathbf l)] (\mathbf l \cdot \mathbf n)f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\
...
$$
\underset{indirect}{L_r(\mathbf p,\mathbf v)}= \int_{\Omega^{+}} L_r(\mathbf o,-\mathbf l,\mathbf p) (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\omega \hfill \\
\mathbf o:反射源 \hfill \\
$$
...
$$
\underset{indirect}{L_r(\mathbf p,\mathbf v)}= \int_{\Omega^{+}} L_r(-\mathbf l,\mathbf p) (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\ 
考虑所有次级光源:\underset{indirect}{L_r(\mathbf p,\mathbf v)}=\int_A L_r(\mathbf q,-\mathbf l,\mathbf p) (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \frac{\mathrm d\mathbf q}{(\mathbf p-\mathbf q)^2} \hfill \\
A:所有次级光源构成的区域 \quad \mathrm d\mathbf q:次级光源\mathbf q附近的微小区域 \hfill \\
\\
...
$$
环境映射:\underset{env}{L_r(\mathbf p,\mathbf v)}= \int_{\Omega^{+}} L(-\mathbf l,\mathbf p)V(\mathbf p,\mathbf l) (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\
\approx \overline{V(\mathbf p)}\int_{\Omega^{+}} L(-\mathbf l,\mathbf p)(\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\
$$
...
$$
\underset{indirect}{L_r(\mathbf p,\mathbf v)}= \int_{\Omega^{+}} L_r(\mathbf q,-\mathbf l)[1-V(\mathbf p,\mathbf l)] (\mathbf l \cdot \mathbf n) f(\mathbf p,\mathbf l,\mathbf v) \mathrm d\mathbf l \hfill \\
