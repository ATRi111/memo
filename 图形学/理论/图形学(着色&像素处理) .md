...
$$
\mathrm{Cook-Torrance}模型:f(\mathbf p,\mathbf l,\mathbf v) = (1-F(\mathbf v,\mathbf h;F_0))\frac{\rho}{\pi} + \frac{D(\mathbf h;\mathbf n,\alpha)F(\mathbf v,\mathbf h;F_0)G(\mathbf l,\mathbf v;\mathbf n,\alpha)}{4 \ (\mathbf l \cdot \mathbf n)(\mathbf v \cdot \mathbf n)} \hfill \\
给定物体表面一点\mathbf p,考虑单一光源:\underset{direct}{L_{r}(\mathbf p,\mathbf v)}= E(-\mathbf l,\mathbf p) (\mathbf l \cdot \mathbf n) \left[(1-F(\mathbf v,\mathbf h;F_0))\frac{\rho}{\pi} + \frac{D(\mathbf h;\mathbf n,\alpha)F(\mathbf v,\mathbf h;F_0)G(\mathbf l,\mathbf v;\mathbf n,\alpha)}{4 \ (\mathbf l \cdot \mathbf n)(\mathbf v \cdot \mathbf n)} \right] \hfill \\
...
\alpha:\mathbf p处的\textbf{粗糙度的平方},[0,1] \quad \rho:\mathbf p处的\textbf{兰伯特反射率} \hfill \\
\\
对于\textbf{特定波长}的光:F_0=\frac{(n_1-n_2)^2}{(n_1+n_2)^2} \hfill \\
F_0:\mathbf p处的\textbf{菲涅尔反射率}  \quad n_1,n_2:反射面两侧两种物质对\textbf{特定波长}的光的\textbf{折射率(IoR)} \hfill \\
$$
...
- **每次反射时，入射角和F项决定了漫反射和镜面反射的能量分配（认为材质各向同性）**
...
$$
\mathrm{Schlick \ Fresnel}:F(\mathbf v,\mathbf h;F_0) \approx \mathrm{Lerp}\{F_0,1,(1 - \mathbf v \cdot \mathbf h)^5\}=F_0 + (1-F_0)(1 - \mathbf v \cdot \mathbf h)^5 \hfill \\
\\
