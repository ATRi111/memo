...
- Microfacet模型中，认为材质不透光，仅考虑最外层表面的反射，**次表面散射被简化成了漫反射（且计算漫反射时不考虑微观法线分布，只使用宏观法线近似计算）**
...
\mathrm{Frostbite \ Diffuse}:f_d(\mathbf p,\mathbf l,\mathbf v)= s(\mathbf l;\mathbf n,r)s(\mathbf v;\mathbf n,r)\mathrm{Lerp}\{1,k_e,r\}\frac{\rho}{\pi} \hfill \\
其中s(\mathbf x;\mathbf n,r)=\mathrm{Lerp}\{f_0,f_{90},(1 - \mathbf x \cdot \mathbf n)^5\} \quad f_0=1 \quad f_{90}=\frac{r}{2}+2(\mathbf v \cdot \mathbf h)^2r  \hfill \\
k_e:经验常数,通常取0.66225,以确保反射后总能量不增加 \hfill \\
\\
...
其中 g(\mathbf x)= \frac{1}{2}\left(\frac{\sqrt{\alpha^2 + (1-\alpha^2)(\mathbf n \cdot \mathbf x)^2}}{\mathbf n \cdot \mathbf x}- 1\right) \hfill \\
\mathrm{Schlick-Smith \ SMT}:G(\mathbf l,\mathbf v;\alpha,\mathbf n) = g(\mathbf l)g(\mathbf v) \quad 其中 g(\mathbf x) = \frac{\mathbf n \cdot \mathbf x}{(1-k)\mathbf n \cdot \mathbf x + k} \quad k = \frac{(\alpha + 1)^2}{8} \hfill \\
$$
