...
$$
给定物体表面一点\mathbf p,考虑单一光源,规定f(\mathbf p,\mathbf l,\mathbf v) = k_d + k_s\frac{(\mathbf n \cdot \mathbf h)^p}{(\mathbf l \cdot \mathbf n)}: \hfill \\
...
- **理论上，余弦项应该使用surface normal计算；实际上，为了充分利用法线贴图和面数有限的模型，通常用shading normal计算**
...
- 总体过程：
  ...
  3. **每个片元计算间接光照时，获取最近的若干个Light Probe，根据其位置得到各probe权重，然后加权平均**
     ...
     - 对于镜面反射，通常只关心观察方向镜面对称方向附近的入射光，在多个probe上分别采样然后加权平均
