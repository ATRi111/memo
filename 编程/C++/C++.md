...
- 根据依赖方式的不同，部分情况下，可以用**前向声明**代替**仅头文件中的**`#include`，以消除或预防循环依赖
  - 源文件中通常没必要使用前向声明，除非单一源文件内部存在相互依赖

- 假设第一个头文件中定义了类A，第二个头文件中前向声明了类A，那么第二个头文件中：
  - 可以定义A类变量（A类**实例除外**）
  - 可以将A用作函数参数/返回值（A类**实例除外**）
  - 不能在函数定义中访问A的成员（在源文件中`#include`，在源文件中定义即可）
  - 不能继承A

- 可以前向声明模板类（含未特化/偏特化/全特化），同样可以将模板类用于变量定义、函数参数/返回值（实例除外）
...
- 过程：
  1. 执行某个系统调用，内核从某个可执行文件创建进程
  2. 内核读取可执行文件的**ELF头**，建立**虚拟内存**（各个段）与物理内存的映射
  3. **动态链接器**把可执行文件依赖的动态链接库加载到内存，并进行重定位
  4. 根据ELF头记录确定程序入口地址，调用相应入口点（通常是`_start`函数，会设置栈区间，并准备好`main`的参数）
  5. **构造全局/静态对象**
- 在同一编译单元内，全局/静态对象的构造顺序与定义顺序相同；跨编译单元的顺序未定义
  - C#的静态构造函数是在首次构造类实例或访问类静态成员时才调用
