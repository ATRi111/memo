...
<img src="image-20240722150117104.png" alt="image-20240722150117104" style="zoom:50%;" />

- 预编译头**以项目为单位**；将项目设置为使用预编译头，然后设置**从项目根路径开始的相对路径**以指明预编译头文件的位置
  - 如果使用预编译头，该项目中所有源文件和头文件必须**首先包含预编译头**
...
- **动态链接库(.dll)仅在exe文件运行时发挥作用，编译器不能访问dll文件**
  - **为了正确地编译，仍然需要引用lib文件**（C++程序使用的dll文件总是伴随着一个lib文件）
...
```c++
#define DEBUGMODE 0
//ifdef 仅区分有无,#if则可以确认某个宏的值(没有任何#define时默认为0)
#if DEBUGMODE == 1			
...
- 主要作用：
  ...
  - 降低在构造函数中构造其他类时的开销
  - **特别地，可以用于初始化常量成员、引用成员，不可用于初始化静态成员**
...
- 会被继承的类，应当定义**虚析构函数**；子类中的析构函数并不会重写父类中的，而是确保二者先后被调用
