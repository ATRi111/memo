...
<img src="Image/image-20240617130439978.png" alt="image-20240617130439978" style="zoom:67%;" />

<img src="Image/image-20240617130738534.png" alt="image-20240617130738534" style="zoom:67%;" />

- 一个解决方案中包含若干个项目
  - 项目有独立的设置、独立的开发语言
  - 通常将其中一个设置为**应用程序（生成.exe）**，其余设置为**库（生成.dll或.lib）**，并**令应用程序引用各个库**
  - 正确设置了项目间引用后，Visual Studio会**自动完成对本解决方案生成的.lib和.dll的引用，但仍然需要设置额外包含目录**（见引用文件）
...
- **静态链接库(.lib)在VS中通过项目设置直接添加**
  - 首先在**项目属性→链接器→常规→附加库目录**中添加库目录
  - 再在**项目属性→链接器→输入→附加依赖项**中指定库目录中的文件
...
- 表示**静态局部变量，静态成员，静态全局变量或函数**

### extern

- 声明其他文件中定义的**全局变量/全局函数/类**
...
- 构造新的shared_ptr时，两个指针指向的地址指向堆上的地址
...
|                  | 左值                          | 左值引用 | 右值引用 | 右值                          |
| ---------------- | ----------------------------- | -------- | -------- | ----------------------------- |
| **类实例左值**   | **分配内存，调用构造函数**[1] | 同左     | 同左     | **分配内存，调用构造函数**[2] |
| **左值引用**     | **新增指针，指向已分配内存**  | 同左     | 同左     | 不允许                        |
| **常量左值引用** | **新增指针，指向已分配内存**  | 同左     | 同左     | **分配内存，调用构造函数**[3] |
| **右值引用**     | 不允许                        | 不允许   | 不允许   | **同上**                      |

*上表中，第一行表示赋值号右侧对象的类型，第一列表示赋值号左侧对象的类型*

[1]：调用哪个构造函数视情况而定，还可能会调用等号重载，不过无论如何都会调用至少一次某种构造函数
...
| **参数类实例** | **调用移动构造函数构造外部变量**[3] | **违法操作**[1] | 同左 | 编译错误 |
| **全局类实例等** | **调用拷贝构造函数构造外部变量**[4] | **同左**                            | **同左** | 编译错误 |

*上表中，第一行为定义函数时规定的返回值类型，第一列则包含实际的返回值的类型、生命周期等信息；规定函数外部均以类实例接收返回值（而不是引用）*
...
- override是说明符，表示该方法可能发生重写，不是必须的，主要作用是帮助编译器检查
- 判断重写、重载的步骤：
