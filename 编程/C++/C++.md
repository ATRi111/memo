...
- 完美转发是指，在模板函数传参时，完美保留参数的**类别（左值/右值，有无const）**
- `std::forward`函数确保参数保持其原本的类别，进而确保调用了正确的函数重载
- 函数重载能在一定程度上实现完美转发的功能，但会使得代码十分臃肿
...
- **虚函数表**：类似数组，其中包含若干个虚函数指针，位于**只读数据段**

- **虚函数指针**：每个指针指向该类中的虚函数实现，本质上是指向**代码段**的地址

- 通常情况下，如果一个类继承N个带有虚函数的类，则具有N个虚函数表
  - 如果其自身也有虚函数，会**和其中一个父类共用虚函数表**（默认第一个）

- 虚函数表的结构
- 有N个虚函数表的类有N个**虚函数表指针**，是类占用的空间的一部分（虚函数表）
- 构造类实例时，**调用哪个构造函数，就令该实例的虚函数表指针指向该类的虚函数表首地址**（赋值给父类指针不改变虚函数表指针，以实现多态）
