...
- **静态链接库(.lib)仅参与编译，编译得到的.exe文件会包含所有静态链接库的内容，无需依赖.lib文件即可运行**
  - **静态链接库(.lib)在VS中通过项目设置直接添加，也可以将lib文件放在生成exe的目录中来让编译器访问（？）**
...
- 成员变量若定义为引用或指针，可避免拷贝，但必须注意其来源：
  - **若来源为外部堆对象地址，必须确保外部在合适的时机人为释放该地址**
  - **若来源为外部栈对象地址，必须确保类使用完毕之前不会被自动释放**
...
| ---------------- | ------------ | --------------------- | -------------- | ----------------- | --------------------------- |
| **静态实例**     | **可行**     | **编译错误**          | **编译错误**   | **链接错误[2]**   | **链接错误[3]**             |
| **静态常量实例** | 同上         | **可能的编译错误[9]** | 同上           | 同上              | 同上                        |
| **静态指针**     | 同上         | **编译错误**          | 同上           | 同上              | 同上                        |
| **静态常量指针** | 同上         | 同上                  | 同上           | 同上              | 同上                        |
| **静态指针常量** | 同上         | 同上                  | 同上           | 同上              | 同上                        |
| **静态引用**     | 同上         | 同上                  | 同上           | 同上              | 同上                        |
| **静态常量引用** | 同上         | 同上                  | 同上           | 同上              | 同上                        |
| **实例**         | **编译错误** | **可行**              | **可行**       | **可行**          | **尝试调用无参构造函数[4]** |
| **常量实例**     | 同上         | 同上                  | 同上           | **编译错误[5]**   | **编译错误[6]**             |
| **指针**         | 同上         | 同上                  | 同上           | **可行**          | **运行时错误[7]**           |
| **常量指针**     | 同上         | 同上                  | 同上           | **编译错误[5]**   | **编译错误[6]**             |
| **引用**         | 同上         | 同上                  | 同上           | **不是初始化[8]** | **编译错误[6]**             |
| **常量引用**     | 同上         | 同上                  | 同上           | **编译错误[5]**   | **编译错误[6]**             |
...
[8]：实质上是重新赋值，必须在其他时机先初始化；对引用重新赋值，实质上是修改引用指向的地址上的内容

[9]：唯独基本数据类型的静态常量实例可以在声明时初始化（？）
