...
- 输入一个数组，输出数组中的**多数元素**（占比**大于**50%的元素）的值
- 输入的数组**总是存在多数元素**
...
- 输入一个非负整数数组`a`，取两个不同的下标`i,j`，令`b=(i-j)*Math.Min(i,j)`
- 输出`b`所有取值中的最大值
- 并不容易发现可以使用贪心算法，即使未能发现，也不妨先尝试
...
public class Solution
{
    ...
    public bool Backtrack(Node p, int val)
    {
...
- 输入一个数组，该数组初始为升序，然后旋转过（在某个位置分为两个部分，交换两个部分的顺序）；输入要查找的数
- 返回查找的数的下标，不存在则返回-1
...
- 输入一个二叉树，和一个数`targetSum`，找出所结点值总和等于`targetSum`的**路径**（将二叉树视为有向图，路径的定义与图相同）
- 输出符合条件的路径的数目
- 溢出问题可以通过取余解决
...
- 输入一个数组，每个元素各不相同
- 输出数组的**全排列**
- 数组元素的具体值无关紧要，关心的是下标的排列顺序；先初始化好第一种排列，然后根据递推地计算下一个排列
...
- 输入一个int数组
- 对于每个元素，找出右边下一个更大的数，计算其下标与当前数下标之差（没有则记为0），结果用int数组输出
- 适合用单调栈解决
